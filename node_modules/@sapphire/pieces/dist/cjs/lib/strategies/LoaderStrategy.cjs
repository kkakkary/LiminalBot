...
var _LoaderStrategy = class _LoaderStrategy {
  constructor() {
    ...
    if (env_cjs.CanLoadTypeScriptFiles) {
  ...
  filter(path$1) {
    ...
    if (this.filterDtsFiles && path$1.endsWith(".d.ts")) return null;
  ...
  async *walk(store, path$1, logger) {
    ...
    try {
      ...
      for await (const item of dir) {
        if (item.isFile()) yield path.join(dir.path, item.name);
        else if (item.isDirectory()) yield* this.walk(store, path.join(dir.path, item.name), logger);
      }
    } catch (error) {
      if (error.code !== "ENOENT") this.onError(error, path$1);
    }
  }
};
