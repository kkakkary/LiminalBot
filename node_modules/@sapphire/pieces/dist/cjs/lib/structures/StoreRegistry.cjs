...
require('../errors/LoaderError.cjs');
...
var _StoreRegistry = class _StoreRegistry extends collection.Collection {
  constructor() {
    super(...arguments);
    /**
     * The queue of pieces to load.
     */
    __privateAdd(this, _pendingManuallyRegisteredPieces, new collection.Collection());
  }
  /**
   ...
   */
  registerPath(rootDirectory = RootScan_cjs.getRootData().root) {
    const root = Path_cjs.resolvePath(rootDirectory);
    for (const store of this.values()) {
      store.registerPath(path.join(root, store.name));
    }
  }
  /**
   * Registers a store.
  ...
  register(store) {
    ...
    if (queue) {
      for (const entry of queue) {
        store[constants_cjs.ManuallyRegisteredPiecesSymbol].set(entry.name, entry);
      }
...
//# sourceMappingURL=StoreRegistry.cjs.map
//# sourceMappingURL=StoreRegistry.cjs.map