...
import { ArgumentError } from '../errors/ArgumentError.mjs';
...
var _Args = class _Args {
  ...
  async restResult(type, options = {}) {
    ...
    const data = join(this.parser.many().unwrapOr([]));
  ...
  nextMaybe(cb) {
    return Option.from(typeof cb === "function" ? this.parser.singleMap(cb) : this.parser.single());
  }
  next(cb) {
  ...
  resolveArgument(arg) {
    ...
    return container.stores.get("arguments").get(arg);
  }
  /**
  ...
  static error(options) {
    return Result.err(new ArgumentError(options));
  }
};
__name(_Args, "Args");
var Args = _Args;
