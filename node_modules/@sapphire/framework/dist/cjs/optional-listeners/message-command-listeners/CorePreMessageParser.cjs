...
var _CoreListener = class _CoreListener extends Listener_cjs.Listener {
  ...
  async run(message) {
    ...
    if (prefix === null) client.emit(Events_cjs.Events.NonPrefixedMessage, message);
    else client.emit(Events_cjs.Events.PrefixedMessage, message, prefix);
  }
  async canRunInChannel(message) {
    if (message.channel.type === discord_js.ChannelType.GroupDM) return false;
